...
namespace bifeldy_sd3_lib_31.Handlers {
    ...
    public class CDbHandler : IDbHandler {
        ...
        public CDbHandler(IOptions<Env> env, IApplication app, IOracle oracle, IPostgres postgres, IMsSQL mssql) {
            _env = env.Value;
            _app = app;
                        ...
                        try {
                            bool res2 = await OraPg.ExecQueryAsync(
                                $@"
                                    ...
                                    VALUES (:kode_dc, :nama_program, :ip_client, :versi, {(_env.IS_USING_POSTGRES ? "NOW()" : "SYSDATE")})
                                ",
                                new List<CDbQueryParamBind> {
